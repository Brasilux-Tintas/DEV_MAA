...
/*/{Protheus.doc} BRSACA01
Atendimento ao CLiente SAC
@type function Rotina
@version  1.00
@author ND -  alterado por marioantonaccio
@since ND - alterado em 17/07/2026
...
User Function BRSACA01()
   ...
   If RetCodUsr() =="001327" // MAA
      ...
      aadd(aRotina, {OemToAnsi("Visualizar")             , "u_SACA01_1"       , 0, 3})
      aadd(aRotina, {OemToAnsi("Atendimento")            , "u_SACA01_2"       , 0, 2})
      aadd(aRotina, {OemToAnsi("Excluir" )               , "u_SACA01_3"       , 0, 4})
      aadd(aRotina, {OemToAnsi("Parecer" )               , "u_SACA01_3"       , 0, 6})
      aadd(aRotina, {OemToAnsi("Encerra" )               , "u_SACA01_3"       , 0, 6})
      aadd(aRotina, {OemToAnsi("Mostrar Pendentes")      , 'U_SACA01_F(1,1)'  , 0, 0})
...
User Function SACA01_2(cAlias, nRecno, nOpc)
   ...
   Endif
   If RetCodUsr() =="001327"
      aVetOpc := {"Novo SAC", "Altera SAC","Re-Abre SAC"}
      nOpcE   := 2
   Endif
   If Len(aVetOpc) > 0
      /*ÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
...
User Function SACA01_1(cAlias, nRecno, nOpc)
   ...
   */
   ...
   If SZQ->ZQ_FLAG $ 'F.R.C.M.A.V.D.E.T.S.#.-' //.and. (nOpc == 6 .or. nOpc == 4)
      ...
      //maa 20260716
      oSayFSac   := TSay():New( 055, 155, {||"Custo Frete:"    }       , oFol1Sac:aDialogs[z], , oFon2Sac, .F., .F., .F., .T., CLR_BLUE, CLR_WHITE, 070, 008)
      ...
      // MAA 20260716
      oGetFSac   := TGet():New( 053, 227, {|u| If(PCount() > 0, nGetFSac := u, nGetFSac)}, oFol1Sac:aDialogs[z], 060, 010, '@E 9999,999.99', , CLR_BLACK, CLR_WHITE, oFon2Sac, , , .T., "", , , .F., .F., , .T., .F., "", "nGetFSac", , )
...
Static Function FGeraEspel(nOpcRes)
   ...
   If nOpcRes ==1
      ...
      If SZQ->ZQ_STATUS =='2' .and. RetCodUsr() <> '001327'
         MsgStop("Este Sac já encontra-se encerrado!", "Atenção")
         Return
      Endif

      // MAA 20260709 -  Selecao de gerar espelho em relatorio ou excel
      If FWAlertNoYes("Deseja gerar o espelho em RELATORIO?","Espelho em RELATORIO")
         FGrEspRel()
         Return(NIL)
      End
      //End MAA 20260709 -  Selecao de gerar espelho em relatorio ou excel
...
Static Function fGravNFSAC()
   ...
   BEGIN TRANSACTION //incluido dyego
      ...
      Else
         ...
         //MAA - Gera NCC
         If Posicione("SF4",1,xFilial("SF4")+cTesEnt,"F4_DUPLIC")  == "S"
            cParcela := AllTrim(Chr(Asc(GetMV("MV_1DUP"))))
            cParcela := PADR(cParcela,TAMSX3("E1_PARCELA")[1]," ")
            While .T.
               //Verifica se ja nao Existe
               SE1->(dbSetOrder(1))
              If  SE1->(MsSeek( FWxFilial("SE1") + SF1->F1_SERIE + SF1->F1_DOC + cParcela +"NCC"))
            ...
            aadd(aVetSE1, {"E1_NUM"    , SF1->F1_DOC                                                                , Nil})
            aadd(aVetSE1, {"E1_PREFIXO", SF1->F1_SERIE                                                              , Nil})
            aadd(aVetSE1, {"E1_PARCELA", cParcela                                                                   , Nil})
            aadd(aVetSE1, {"E1_TIPO"   , "NCC"                                                                      , Nil})
            aadd(aVetSE1, {"E1_NATUREZ", Posicione("SA1",1,xFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA,"A1_NATUREZ"), Nil})
            aadd(aVetSE1, {"E1_CLIENTE", SF1->F1_FORNECE                                                            , Nil})
            aadd(aVetSE1, {"E1_LOJA"   , SF1->F1_LOJA                                                               , Nil})
            aadd(aVetSE1, {"E1_NOMCLI" , Posicione("SA1",1,xFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA,"A1_NREDUZ") , Nil})
            aadd(aVetSE1, {"E1_EMISSAO", SF1->F1_EMISSAO                                                            , Nil})
            aadd(aVetSE1, {"E1_VENCTO" , SF1->F1_EMISSAO                                                            , Nil})
            aadd(aVetSE1, {"E1_VENCREA", DataValida(SF1->F1_EMISSAO,.T.)                                            , Nil})
            aadd(aVetSE1, {"E1_VALOR"  , SF1->F1_VALBRUT                                                            , Nil})
            aadd(aVetSE1, {"E1_HIST"   , "Ref.SAC "+cGet1Sac                                                                        , Nil})
            ...
            If lMsErroAuto
               erroTrans := .T.
               If Aviso("Pergunta", "NCC não gerado. Deseja visualizar o log?", {"Sim","Nao"}, 1, "Atencao") = 1
                  _ocorreErr := .F.
                  DisarmTransaction()  //incluido dyego
                  MostraErro()
               Endif
...
Static Function FGrEspR02()
   ...
   BeginSQL ALIAS "TCR"
      SELECT
         ...
         ROUND(SUM(SZR.ZR_QTD * SD2.D2_PRCVEN) 
               + SUM(SD2.D2_VALIPI / (SD2.D2_QUANT / SZR.ZR_QTD))  
               + SUM(SD2.D2_ICMSRET / (SD2.D2_QUANT / SZR.ZR_QTD))
               + SUM(SD2.D2_VALFRE / (SD2.D2_QUANT / SZR.ZR_QTD)),2) AS TOTNOTA,
   ...
   BeginSQL Alias "TCQ"
      SELECT
         ...
         ROUND((SD2.D2_VALFRE /(SD2.D2_QUANT / SZR.ZR_QTD)), 2) AS FRETE,
   ...
   nLinha:=810
   While .NOT. TCQ->(EOF())
      // If Substr(TCQ->TES,1,1) == "G"
      //  lGRU:=.T.
      // End

      //Pega TES de devolucao - SAIDA
      cCFOP:=TCQ->CFOP
      SF4->(dbSetOrder(1))
      SF4->(dbSeek(xFilial("SF4")+TCQ->TES))
      If .NOT. Empty(SF4->F4_TESDV)
         cCFOP:=Posicione("SF4",1,xFilial("SF4")+SF4->F4_TESDV,"F4_CF")
      End

      If TCQ->EST <> GetMv("MV_ESTADO")
         cCFOP:="6"+Substr(cCFOP,2,5)
         lGRU:=.T.
      Else
         cCFOP:="5"+Substr(cCFOP,2,5)      
      End
      ...
      oPrn:Say(nLinha,1345,cCFOP ,oFont12,,,PAD_LEFT)
   ...
   If lGRU
      LINHA(@nLinha)
      LINHA(@nLinha)
      cTexto:="Mercadoria deverá ser devolvida ao CD BRASILUX localizado em Guarulhos - SP:"
      oPrn:Say(nLinha,0030,OemToAnsi(cTexto),oFont14N)
      LINHA(@nLinha)
      cTexto:="Endereço: RUA ANTONIO RODRIGUES FILHO, 501 (VILA AEROPORTO) - CEP: 07170-650 - GUARULHOS - SP"
      oPrn:Say(nLinha,0030,OemToAnsi(cTexto),oFont14N)
   End
   oPrn:EndPage()
