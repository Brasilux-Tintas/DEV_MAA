...
//oCBox1Lo   := TComboBox():New( 062, 290, {|u| If(PCount() > 0, nCBox1Lo := u, nCBox1Lo)}, {"","FAB1","FAB2","ARA","DEPO"}, 060, 016, oGrp2Env, , , , CLR_BLACK, CLR_WHITE, .T., oFon2Env, "", , , , ,  , , nCBox1Lo ) 
	/*Adicionado por Alex - 09-10-2025. Chamado R9449*/
	oCBox1Lo   := TComboBox():New( 062, 290, {|u| If(PCount() > 0, nCBox1Lo := u, nCBox1Lo)}, {"","ARA","FAB1","FAB2","DEPO"}, 060, 016, oGrp2Env, , , , CLR_BLACK, CLR_WHITE, .T., oFon2Env, "", , , , ,  , , nCBox1Lo ) 
...
Static Function fValCodPro(nOpcMnt)  //1 - Montagem do Pallet 2- Desmontagem
    ...
    Local nQtdAtual := 0
    Local aGerEtq   := {}
	/*Adicionado por Alex - 08-10-2025. Chamado R9449*/
    	...
    	If Found() 
	    	While !lMandarEmail .and. !(SC2->(Eof())) .AND. (XFilial("SC2") == SC2->C2_FILIAL) .AND. (Substr(cGet1Mnt,1,6) == Alltrim(SC2->C2_LOTE)) 
				If Substr(cGet1Mnt,7,2) == Substr(SC2->C2_PRODUTO,11,2) 
					...
					/*Alterado por Alex - 09-10-2025. Chamado R9449*/
					// dDtValida:= u_ValidProd(SC2->C2_LOTE,SC2->C2_PRODUTO)
					dDtValida:= ctod(cValToChar(u_ValidProd(SC2->C2_LOTE,SC2->C2_PRODUTO)))
					If Empty(dDtValida)
						Alert("Produto: " + SC2->C2_PRODUTO + " | Sem data de validade definida!")
					Else
						nPrazo := dDtValida - dDataBase
					EndIf
					/*Alterado por Alex até aqui- 09-10-2025. Chamado R9449*/
					...
					// If nPrazo > nValida 
					// 	Messagebox("Produto bipado vencerá em "+cValtoChar(nPrazo)+" dias","Atenção...",48) 				
  					// /*Adicionado por Alex - 09-10-2025. Chamado R9449*/	
					// Else
					//    	Messagebox("Produto bipado vencido e portanto não poderá ser comprado, entre em contato com o Supervisor  !!","Atenção...",48) 
					//     //cAuxMens := "Produto bipado vencerá em "+cValtoChar(nPrazo)+" dias e portanto não poderá ser comprado !"+chr(13)+chr(10)
					// 	cAuxMens := "Produto bipado vencido e portanto não poderá ser comprado !"+chr(13)+chr(10)
					// 	/*adicionado por Alex até aqui - 09-10-2025. Chamado R9449*/
				    //     cAuxMens +="Lote...............:  "+(SC2->C2_LOTE)+chr(13)+chr(10)
					// 	cAuxMens +="Produto............:  "+SC2->C2_PRODUTO+chr(13)+chr(10)
					...
					// 	Return
					// Endif
					/*Alterado por Alex - 14-10-2025. Chamado R9449*/	
					If nPrazo <= 0 
						Messagebox("Produto bipado vencido e portanto não poderá ser comprado, entre em contato com o Supervisor  !!","Atenção...",48) 
					    //cAuxMens := "Produto bipado vencerá em "+cValtoChar(nPrazo)+" dias e portanto não poderá ser comprado !"+chr(13)+chr(10)
						cAuxMens := "Produto bipado vencido e portanto não poderá ser comprado !"+chr(13)+chr(10)
					...
					Elseif nPrazo <= nValida
						Messagebox("Produto bipado vencerá em "+cValtoChar(nPrazo)+" dias","Atenção...",48)
					Else
						ConOut("Nenhuma ação necessaria!")
						
					Endif
					/*Alterado por Alex até aqui- 14-10-2025. Chamado R9449*/
				Endif
				DbSelectArea("SC2")
				DbSkip()        	
	  		Enddo
