...
User Function BRQIEC01()
   ...
   // MAA - Chamado R13225 - 20260727
   oBtn1IQE   := TButton():New( 015, 250, "Exp.Excel"  , oGrp1IQF, { || FExpEXCEL()}, 037, 012, , , , .T., , "", , , , .F. )
   // MAA - End Chamado R13225 - 20260727

   oTbl1IQF()
   DbSelectArea("TMP")
   DbSetOrder(1)
   oBrw1IQF   := MsSelect():New( "TMP", "", "", { {"TMP_CODF", "", "Cod. Fornec.", ""}, {"TMP_NOMF", "", "Nome Fornec.", ""}, {"TMP_CODP", "", "Cod. Produto", ""}, {"TMP_NOMP", "", "Descricao", ""}, {"TMP_IIQF", "", "IQF", "999"}, {"TMP_CLAS", "", "Classificação", "@!"} }, .F., , {036, 004, 204, 340}, , , oDlg1IQF )
...
Exportação em Excel - Chamado  R13225
...
Static Function FExpEXCEL()
   MsgRun("Processando...","Aguarde...Gerando Registros para EXCEL",{|| FExpEXC01()} )
Return (NIL)

/*/{Protheus.doc} FExpEXC01
Funcao Exportação em Excel - Chamado  R13225
@type function Processamento
@version  1.00
@author marioantonaccio
@since 27/07/2026
@return Character, sem retorno
/*/
Static Function FExpEXC01()

   Local cArquivo := "IQF" //GetTempPath()+'IQF'
   Local cNArq    := " " //GetTempPath()+'IQF'
   Local cDirIMp  := ""
   Private oFWMsExcel

   U_FDirImp(@cArquivo,@cDirImp)

   oFWMsExcel := FWMsExcelXlSX():New()

   oFWMsExcel:AddworkSheet("IQF")

   //Criando a Tabela
   oFWMsExcel:AddTable("IQF","IQF")

   oFWMsExcel:AddColumn("IQF","IQF","Cod.Fornecedor",1,1,.F.)
   oFWMsExcel:AddColumn("IQF","IQF","Nome Fornecedor",1,1,.F.)
   oFWMsExcel:AddColumn("IQF","IQF","Cod.Produto",1,1,.F.)
   oFWMsExcel:AddColumn("IQF","IQF","Descrição",1,1,.F.)
   oFWMsExcel:AddColumn("IQF","IQF","IQF",3,2,.F.,"999")
   oFWMsExcel:AddColumn("IQF","IQF","Classificação",1,1,.F.)

   TMP->(dbGoTop())
   While .NOT. TMP->(EOF())
      oFWMsExcel:AddRow("IQF","IQF",{TMP->TMP_CODF,TMP->TMP_NOMF,TMP->TMP_CODP,TMP->TMP_NOMP,TMP->TMP_IIQF,TMP->TMP_CLAS})
      TMP->(dbSkip())
   End
   TMP->(dbGoTop())
   ...
   cNArq:=Substr(cArquivo,AT( "IQF",cArquivo ),Len(cArquivo))

   FWAlertInfo("Arquivo "+cNArq+" Salvo na Pasta "+AllTrim(cDirImp),"Local Arquivo")

   //Verifica se o Excel está instalado
   If .NOT. ApOleClient("MsExcel")
      If FWAlertYesNo("O Excel esta instalado na máquina!...Deseja Tentar Abrir ?", "Sem Excel")
         WinExec( 'RunDLL32.EXE shell32.dll,ShellExec_RunDLL "' + cArquivo+".XLSX"+ '"' ,1)
      Else
         Return(NIL)
      End
   EndIf
...
User Function FDIRIMP(cArquivo,cDirImp)

   Local cTempo:=StrTran(Time(),":","")

   //Pega Diretorio de Impressaõ Usuario
   If Select("XPRT") > 0
      XPRT->(dbCloseArea())
   End
   BEGINSQL Alias "XPRT"
      ...
      FROM
         SYS_USR_PRINTER 
      WHERE
         USR_ID = %EXP:RetCodUsr()%
   ENDSQL
   cDirImp:=AllTrim(XPRT->USR_DIRIMP)
   XPRT->(dbCloseArea())

   //se tiver vazio, Cria TEMP
   If Empty(cDirImp)
      cDirImp:="C:\TEMP\"      
   End   

   //Se nao Existir Cria - Pode Ser Tanto Lcoal quanto na rede - quem maanda é a variavel
   If .NOT. (ExistDir(cDirImp))
      MakeDir(cDirImp)
   End
   
   cArquivo:=cDirImp+"IQF"+"_"+cTempo+".XLSX"
