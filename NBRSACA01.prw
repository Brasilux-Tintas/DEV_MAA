...
@since 28/05/2026
@return character, sem reteonor
/*/
User Function BRSACA01()

   Private cUserDir  := GETMV("ZP_PAR0207")           as Character
   ...
   If u_fVerAcsUsr(cRotUsu, 2) .and. cDepart $ 'INF/ATE/DIR'
      If cDepart == 'DIR' .and. cCodUsr $ _cUserDir
   ...
   If cCodUsr $ '000000/000071/000092/000023'
   ...
   if cCodUsr == '001327'
      ...
      aadd(aRotina, {OemToAnsi("Visualizar")             , "u_SACA01_1()"     , 0, 2})
      aadd(aRotina, {OemToAnsi("Parecer/Encerra/Excluir"), "u_SACA01_3()"     , 0, 6})
      aadd(aRotina, {OemToAnsi("Atendimento" )           , "u_SACA01_2()"     , 0, 3})
      aadd(aRotina, {OemToAnsi("Mostrar Pendentes")      , 'U_SACA01_F(1,1)'  , 0, 0})
...
User Function SACA01_2(cAlias, nRecno, nOpc)
   ...
   If Len(aVetOpc) > 0
      ...
      aAdd(aPergs, {2, "Selecione a Opção:", nTipo, aVetOpc,80, ".T.", .F.})

      If ParamBox(aPergs, "Informe os parâmetros")
         nOpc:=If(nTipo == 1,3,4)
         If .NOT. FWAlertYesNo("Versao Nova(SIM) Versao Antiga (Nao)", "Continuar")
...
User Function SACA01_1(cAlias, nRecno, nOpc)
   ...
   Local aFol1Sac     := {}        as Array
   Local nOpcSac      := 0         as Numeric
   Local nY           := 0         as Numeric
   Private aCoBrw1Sac := {}        as Array
   Private aCoBrw2Sac := {}        as Array
   Private aCpoAGd    := {}        as Array
   Private aCpoAGd2   := {}        as Array
   Private aHoBrw1Sac := {}        as Array
   Private aHoBrw2Sac := {}        as Array
   Private aHoBrw3Sac := {}        as Array
   Private aPageSac   := {}        as Array
   Private cGet10Sac  := Space(06) as character
   Private cGet11Sac  := Space(06) as character
   Private cGet12Sac  := Space(06) as character
   Private cGet1Sac   := " "       as character
   Private cGet3Sac   := " "       as character
   Private cGet4Sac   := " "       as character
   Private cGet5Sac   := " "       as character
   Private cGet6Sac   := " "       as character
   Private cGet7Sac   := " "       as character
   Private cGet8Sac   := " "       as character
   Private cGet9Sac   := " "       as character
   Private cGetASac   := " "       as character
   Private cGetBSac   := " "       as character
   Private cGetCSac   := " "       as character
   Private cGetDSac   := " "       as character
   Private cMul1Sac   := " "       as character
   Private cMul2Sac   := " "       as character
   Private cMul3Sac   := " "       as character
   Private cOpcBPro   := " "       as character
   Private cSay1Sac   := Space(01) as character
   Private cSay2Sac   := Space(01) as character
   Private cSay3Sac   := Space(01) as character
   Private cSay4Sac   := Space(01) as character
   Private cSay5Sac   := Space(01) as character
   Private cSay6Sac   := Space(01) as character
   Private cSay7Sac   := Space(01) as character
   Private cSay8Sac   := Space(01) as character
   Private cSayASac   := Space(01) as character
   Private cSayCSac   := Space(01) as character
   Private dGet2Sac                as Date
   Private nCBo1Sac   := " "       as character
   Private nCBo2Sac   := " "       as character
   Private nCBo3Sac   := " "       as character
   Private nGetESac   := 0         as numeric
   Private nGetFSac   := 0         as numeric
   Private nGetGSac   := 0         as numeric
   Private nGetHSac   := 0         as numeric
   Private nGetISac   := CToD(" ") as Date
   Private noBrw1Sac  := 0         as numeric
   Private noBrw2Sac  := 0         as numeric
   Private oMul1Sac   := Nil       as Object
   Private oMul2Sac   := Nil       as Object
   Private oMul3Sac   := Nil       as Object
   //Devolução (Folder 5)
   Private cGetRSac := space(100) as character
   Private cSayNSac := " "        as character
   Private nGetKSac := space(09)  as character
   Private nGetLSac := space(03)  as character
   Private nGetMSac := CToD(" ")  as Date
   Private nGetQSac := space(45)  as character
   Private oBtn1Dev := Nil        as Object
   Private oBtn2Dev := Nil        as Object
   Private oBtn3Dev := Nil        as Object
   Private oBtn4Dev := Nil        as Object
   Private oBtn5Dev := Nil        as Object
   Private oBtn6Dev := Nil        as Object
   Private oGetESac := Nil        as Object
   Private oGetFSac := Nil        as Object
   Private oGetLSac := Nil        as Object
   Private oGetMSac := Nil        as Object
   Private oGetOSac := Nil        as Object
   Private oSayKSac := Space(01)  as character
   Private oSayLSac := Space(01)  as character
   Private oSayMSac := Space(01)  as character
   Private oSayNSac := Space(01)  as character
   //Resumo (Folder 6)
   Private nGetRSac   := 0         as Numeric
   Private nGetSSac   := 0         as Numeric
   Private nGetTSac   := 0         as Numeric
   Private nGetUSac   := 0         as Numeric
   Private nGetVSac   := 0         as Numeric
   Private nGetXSac   := 0         as Numeric
   Private nGetYSac   := 0         as Numeric
   Private oBrw1Sac   := Nil       as Object
   Private oBtn1Sac   := Nil       as Object
   Private oBtn2Sac   := Nil       as Object
   Private oBtn3Sac   := Nil       as Object
   Private oBtn4Sac   := Nil       as Object
   Private oBtn5Sac   := Nil       as Object
   Private oBtn6Sac   := Nil       as Object
   Private oCBo1Sac   := Nil       as Object
   Private oCBo2Sac   := Nil       as Object
   Private oCBo3Sac   := Nil       as Object
   Private oDlg1Sac   := Nil       as Object
   Private oFld1Sac   := Nil       as Object
   Private oFon10Sac  := Nil       as Object
   Private oFon12Sac  := Nil       as Object
   Private oFon1Sac   := NIL       as object
   Private oFon2Sac   := Nil       as object
   Private oGet10Sac  := Nil       as Object
   Private oGet11Sac  := Nil       as Object
   Private oGet12Sac  := Nil       as Object
   Private oGet1Sac   := Nil       as Object
   Private oGet2Sac   := Nil       as Object
   Private oGet3Sac   := Nil       as Object
   Private oGet4Sac   := Nil       as Object
   Private oGet5Sac   := Nil       as Object
   Private oGet6Sac   := Nil       as Object
   Private oGet7Sac   := Nil       as Object
   Private oGet9Sac   := Nil       as Object
   Private oGetASac   := Nil       as Object
   Private oGetBSac   := Nil       as Object
   Private oGetCSac   := Nil       as Object
   Private oGetDSac   := Nil       as Object
   Private oGetRSac   := Nil       as Object
   Private oGetSSac   := Nil       as Object
   Private oGetTSac   := Nil       as Object
   Private oGetUSac   := Nil       as Object
   Private oGetVSac   := Nil       as Object
   Private oGetXSac   := Nil       as Object
   Private oGetYSac   := Nil       as Object
   Private oGrp10Sac  := Nil       as Object
   Private oGrp11Sac  := Nil       as Object
   Private oGrp12Sac  := Nil       as Object
   Private oGrp2Sac   := Nil       as Object
   Private oGrp3Sac   := Nil       as Object
   Private oGrp4Sac   := Nil       as Object
   Private oSay10Sac  := Nil       as Object
   Private oSay12Sac  := Nil       as Object
   Private oSay1Sac   := Nil       as Object
   Private oSay2Sac   := Nil       as Object
   Private oSay3Sac   := Nil       as Object
   Private oSay4Sac   := Nil       as Object
   Private oSay6Sac   := Nil       as Object
   Private oSay7Sac   := Nil       as Object
   Private oSay8Sac   := Nil       as Object
   Private oSay9Sac   := Nil       as Object
   Private oSayASac   := Nil       as Object
   Private oSayCSac   := Nil       as Object
   Private oSayDSac   := Nil       as Object
   Private oSayRSac   := Nil       as Object
   Private oSaySSac   := Nil       as Object
   Private oSayTSac   := Nil       as Object
   Private oSayUSac   := Nil       as Object
   Private oSayVSac   := Nil       as Object
   Private oSayXSac   := Nil       as Object
   Private oSayYSac   := Nil       as Object
   Private oTempTbl01 := Nil       as Object
   Private oTempTbl02 := Nil       as Object
   Private oTempTbl03 := Nil       as Object
   Private oTempTbl04 := Nil       as Object
   Private oTempTbl05 := Nil       as Object

   aFol1Sac := Iif(nOpc == 6, Iif(nOpc == 6 .and. SZQ->ZQ_FLAG $ 'R.M.A' , {"Itens", "Custos", "Parecer", "Histórico", "Devolução", "Resumo" }, {"Itens", "Custos", "Parecer", "Histórico","Resumo"} ), {"Itens", "Custos", "Diagnóstico/Solução", "Histórico","Resumo"})
   nOpcSac  := Iif(nOpc == 2 .or. nOpc == 6, 0, Iif(nOpc == 4 .and. SZQ->ZQ_FLAG $ 'A.M' , GD_INSERT+GD_DELETE+GD_UPDATE, GD_INSERT+GD_DELETE+GD_UPDATE))
   cGet1Sac := Iif(nOpc == 3, U_NUMSEQLOT("SZQ", 6), SZQ->ZQ_NUM )
   dGet2Sac := Iif(nOpc == 3, dDataBase , SZQ->ZQ_DATA )
   cGet3Sac := Iif(nOpc == 3, space(6) , SZQ->ZQ_CLIENTE )
   cGet4Sac := Iif(nOpc == 3, Space(40) , Posicione("SA1", 1, xFilial("SA1")+cGet3Sac, "A1_NOME") )
   cGet5Sac := Iif(nOpc == 3, Space(20) , Posicione("SA1", 1, xFilial("SA1")+cGet3Sac, "A1_DDD")+ '-' +Posicione("SA1", 1, xFilial("SA1")+cGet3Sac, "A1_TEL") )
   cGet6Sac := Iif(nOpc == 3, Space(20) , Posicione("SA1", 1, xFilial("SA1")+cGet3Sac, "A1_CONTATO") )
   cGet7Sac := Iif(nOpc == 3, cUserName , SZQ->ZQ_ATENDEN)
   cGet8Sac := Iif(nOpc == 3, Time() , SZQ->ZQ_HINI)
   cGet9Sac := Iif(nOpc == 3, Space(3) , Iif(Len(Alltrim(Posicione("AC9", 2, xFilial("AC9")+ 'SZQ' +xFilial("SZQ")+SZQ->ZQ_NUM, "AC9_CODENT"))) <>0, "SIM","NÃO"))
   cGetASac := Iif(nOpc == 3, Space(6) , Posicione("SZR", 1, xFilial("SZR")+SZQ->ZQ_NUM , "ZR_ASSUNTO") )
   cGetBSac := Iif(nOpc == 3, Space(15) , Posicione("SX5", 1, xFilial("SX5")+ 'T1' +cGetASac, "X5_DESCRI") )
   cGetCSac := Iif(nOpc == 3, Space(6) , Posicione("SZR", 1, xFilial("SZR")+SZQ->ZQ_NUM , "ZR_OCORREN") )
   cGetDSac := Iif(nOpc == 3, Space(30) , Posicione("SU9", 1, xFilial("SU9")+cGetASac+cGetCSac, "U9_DESC") )
   nGetESac := Iif(nOpc == 3, 0 , SZQ->ZQ_CUSDEV)
   nGetFSac := Iif(nOpc == 3, 0 , SZQ->ZQ_CUSFRE)
   nGetGSac := Iif(nOpc == 3, 0 , SZQ->ZQ_CUSFIN)
   nGetHSac := Iif(nOpc == 3, 0 , SZQ->ZQ_CUSDEV + SZQ->ZQ_CUSFRE + SZQ->ZQ_CUSFIN)
   cMul1Sac := Iif(nOpc == 3, , Posicione("SZS", 1, xFilial("SZS")+SZQ->ZQ_NUM, "ZS_PARECER") )
   nCBo1Sac := Iif(nOpc == 3, 'Reclamação' , Iif(SZQ->ZQ_TIPOSAC == '1' , 'Reclamação' , Iif(SZQ->ZQ_TIPOSAC == '2' , 'Sugestão' , Iif(SZQ->ZQ_TIPOSAC == '3' , 'Comodato' , Iif(SZQ->ZQ_TIPOSAC == '4' , 'Outros' , 'Interno' ) ) ) ) )
   nCBo3Sac := Iif(nOpc == 3, 'Sem Dev.' , Iif(SZQ->ZQ_TPABAT == '2' , 'NCC' , Iif(SZQ->ZQ_TPABAT == '3' , 'AB-' , Iif(SZQ->ZQ_TPABAT == '4' , 'NCC e AB-' , '1' ) ) ) )
   cSayNSac := Iif(nOpc == 3, Space(15), Iif(Empty( Alltrim(SZQ->ZQ_FNC) ), Space(15), TransForm(Alltrim(SZQ->ZQ_FNC), "@R 99999999999/9999") ) )
   ...
   ElseIf nOpc == 4 //ALTERAÇÃO / RE-ABERTURA DE SAC
      ...
      If SZQ->ZQ_STATUS == '1'
         ...
         Endif
      ElseiF SZQ->ZQ_STATUS == '2'
         If .NOT. cDepart $ 'ATE/INF/DIR' //Re-abre o SAC
   ...
   ElseIf nOpc == 5 //EXCLUSÃO DO SAC
      If SZQ->ZQ_STATUS == '1'
         If .NOT. SZQ->ZQ_FLAG $ 'A/M'
            FWAlertError("O Status desse SAC não permite a exclusão.","Sem Permissao")
            Return (nil)
         Endif
      Else
   ...
   ElseIf nOpc == 6 //PARECER DO SAC
      If SZQ->ZQ_STATUS == '2'
         FWAlertError("O Status desse SAC não permite informar o parecer.", "Parecer nao Permitido")
         Return (NIL)
      Else
         nPosSAC := aScan( aEnvSac, { |x| Alltrim(x[1])== cDepart} )
         If nPosSAC > 0
            If aEnvSac[nPosSAC][1] == cDepart
               If .NOT. aEnvSac[nPosSAC][2] $ SZQ->ZQ_FLAG
                  FWAlertError("O Status desse SAC não permite informar o parecer.", "Parecer nao Permitido")
                  Return (NIL)
               Endif
            Else
         ...
         Else
            If .NOT. SZQ->ZQ_FLAG $ 'A/M'
               If cDepart <> 'INF'
                  FWAlertError("O Status do departamento do usuário não permite informar o parecer.", "Parecer nao Permitido")
                  Return (NIL)
               Endif
            Else
               FWAlertError("O Status desse SAC não permite informar o parecer.", "Parecer nao Permitido")
   ...
   ElseIf nOpc == 8 //ENCERRAMENTO DO SAC
      If SZQ->ZQ_STATUS == '1'
         If .NOt.SZQ->ZQ_FLAG $ 'A/M/F'
            FWAlertError("O Status desse SAC não permite o encerramento.", "Encerramento nao Permitido")
            Return (NIL)
         Endif
         cVarATE := ""
         cVarDIA := ""
         SZS->(DbSetOrder(1))
         If SZS->(DbSeek(xFilial("SZS")+cGet1Sac))
            While .NOT. SZS->(EOF()) .and. SZS->ZS_NUM == cGet1Sac
               If SZS->ZS_LOG $ 'ATE'
            ...
            EndDo
         Endif
         If .NOT. Empty(Alltrim(cVarDIA))
            cMul2Sac := cVarDIA
         Else
            If .NOT. Empty(Alltrim(cVarATE))
               cMul2Sac := cVarATE
            Endif
         Endif
      Else
   ...
   oGetCSac   := TGet():New( 086, 200, {|u| If(PCount() > 0, cGetCSac := u, cGetCSac)}, oGrp4Sac, 028, 010, '', , CLR_BLACK, CLR_WHITE, oFon2Sac, , , .T., "", , , .F., .F., , Iif(nOpc==3, .F., .F.), .F., "SU9", "cGetCSac", , )
...
Static Function fReaReplac()
   ...
   nPosQTD     := aScan(oBrw1Sac:aHeader, {|x| Alltrim(x[2]) == "ZR_QTD"     } )
   nPosQAPRRT  := aScan(oBrw1Sac:aHeader, {|x| Alltrim(x[2]) == "ZR_QAPRRET" } )
...
Static Function fValNota()
   If Empty(cGet3Sac)
      If Empty(cGet10Sac)
         MsgStop("Numero da Nota fiscal não pode ser vazio, verifique!!!!")
         cGet10Sac := space(06)
         cGet12Sac := space(03)
         cGet11Sac := space(06)
         cGet3Sac  := space(06)
...
User Function NSACA01_1(nOpc)
   ...
   Private oSACAtend  := NIL                          as Object
   Private oSACClie   := NIL                          as Object
   Private oSACContat := NIL                          as Object
...
nTPSac
   ...
   */
   If nOpc == 3
      cSacNum   := GetSxeNum("SZQ","ZQ_NUM")  //U_NUMSEQLOT("SZQ", 6)
      cSACAtend := cUserName
      dSACData  := Date()
      cSACHI    := Time()
      cTpDesc   := aItDesc[1]
   ...
   oDlgSAC    := MSDialog():New( 070,232,700,1320,"Atendimento ao Cliente "+If(nOpc==3," - Novo"," - Re-Abertura"),,,.F.,,,,,,.T.,,,.T. )
   ...
   oSay10     := TSay():New(064, 376, {||"Fones"}                                  , oSACGrp02,    ,    , .F., .F., .F.      , .T.      , CLR_BLACK, CLR_WHITE, 032, 008)
   ...
   oSACLj     := TGet():New(072, 180, {|u| If(PCount()>0,cSACLj:=u,cSACLj)}        , oSACGrp02, 016, 008, '' ,    , CLR_BLACK, CLR_WHITE,oFontSubN           ,          ,    , .T., "", , , .F., .F., , .F., .F., "", "cSACLj"    , ,)
   ...
   oSACContat := TGet():New(072, 440, {|u| If(PCount()>0,cSACContat:=u,cSACContat)}, oSACGrp02, 060, 008, '' ,    , CLR_BLACK, CLR_WHITE,oFontSubN           ,          ,    , .T., "", , , .F., .F., , .F., .F., "", "cSACContat", ,)
   ...
   oSACAtend := TGet():New(120, 016, {|u| If(PCount()>0,cSAcAtend:=u,cSAcAtend)}, oSACGrp03, 070, 008, '' ,    , CLR_BLACK, CLR_WHITE,oFontSubN           ,          ,    , .T., "", , , .F., .F., , .F., .F., "", "cSAcAtend", ,)
   oSACHI    := TGet():New(120, 090, {|u| If(PCount()>0,cSACHI:=u,cSACHI)}      , oSACGrp03, 050, 008, '' ,    , CLR_BLACK, CLR_WHITE,oFontSubN           ,          ,    , .T., "", , , .F., .F., , .F., .F., "", "cSACHI"   , ,)
   ...
   //oBrwIte    := MsNewGetDados():New(004,004,060,512,nOpc,'AllwaysTrue()','AllwaysTrue()','',{"ZR_SEQITEM.ZR_PRODUTO.ZR_DESCPRO.ZR_QTD.ZR_QTDORIG.ZR_PEDIDO.ZR_NUMNF.ZR_SERNF.ZR_VUORI.ZR_VTORI.ZR_LOTE.ZR_DTVALID.ZR_QTDPRO.ZR_RETEN.ZR_ITEMDEV.ZR_QAPRRET.ZR_ITEMORI.ZR_QTDABAT'"},0,99,'AllwaysTrue()','','AllwaysTrue()',oaDialogs1,aHoBrwIte,aCoBrwIte )
...
Static Function fNValNota(cNota)

   Local lRet:=.T.
   Local aArea:=FWGetArea()

   If Empty(cNota)
      FWAlertError("Numero da Nota fiscal não pode ser vazio, verifique!!!!","Nao Vazio")
      cSACNF := Space(09)
      cSACSer:=Space(03)
      oSACNF:SetFocus()
      lRet:=.F.
   ...
   If lRet
      SF2->(dbSetOrder(1))
      If SF2->(DbSeek(xFilial("SF2")+cNota, .T.))
         cSACSeri := SF2->F2_SERIE
         cSACCLie := SF2->F2_CLIENTE
         cSACLj   := SF2->F2_LOJA
         SA1->(DbSetOrder(1))
         If SA1->(DbSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))
            cSACNome   := SA1->A1_NOME
            cSACFone   := SA1->A1_DDD+ '-' +SA1->A1_TEL
            cSACContat := SA1->A1_CONTATO
         Else
            FWAlertError("Cliente não encontrado, verifique!!!!","Nao Encontrado")
            lRet:=.F.
         EndIf
      Else
         FWAlertError("Numero da Nota fiscal não encontrado, verifique!!!!","Nao Encntrado")
         cSACNF := space(09)
         lRet:=.F.
      Endif
   Endif
...
Static Function fNValClie(cCliente,cLoja)
   Local lRet:=.T.
   Local aArea:=FWGetArea()

   If Empty(cCLiente)
      FWAlertError("Codigo do cliente não pode ser vazio, verifique!!!!","Nao Vazio")
      cSACClie := space(06)
      cSACLJ  := space(02)
      cSACNome := space(40)
      cSACFone := space(20)
...
Static Function fNValAssun(cAssunto)
   Local aRetInf := FwGetSX5( 'T1', cAssunto )
   Local aArea   := FWGetArea()
   Local lRet    := .T.

   If Len( aRetInf ) > 0
      cSACDAss := RTRIM( aRetInf[1][4] )
   Else
      FWAlertError("Assunto não encontrado, verifique!!!!","Nao Encntrado")
      cSACDAss := space(30)
      lRet:=.F.
      oSACAssu:SetFocus()
   Endif
...
Static Function fNValOcorre(cOcorr)

   Local aArea   := FWGetArea()
   Local lRet:=.T.

   If Empty(cOcorr)
      FWAlertError("Ocorrencia não pode ser vazio, verifique!!!!","Nao Vazio")
      oSACOcor := space(06)
      lRet:=.F.
      oSACOcor:SetFocus()
   End
   ...
   If lRet
      SU9->(DbSetOrder(1))
      If SU9->(DbSeek(xFilial("SU9")+cSACAssu+cOcorr, .t.))
         cSACDOco := RTRIM(SU9->U9_DESC)
         oSACDesc:SetFocus()
      Else
         lRet:=.F.
         FWAlertError("Ocorrencia não encontrada, verifique!!!!","Nao Encontrada")
         cSACOcor := Space(06)
         cSACDOco := Space(30)
         oSACOcor:SetFocus()
      Endif
...
Static Function N_fGrvSac(nOpc)

   Local nPosSEQ := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_SEQITEM" } )
   Local nPosPRO := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_PRODUTO" } )
   Local nPosQTD := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QTD"     } )

   //alterado dyego
   Local nPosQAPRRT := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QAPRRET" } )
   Local nPosITEMOR := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_ITEMORI" } )

   Local nPosQTDABA := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QTDABAT" } )

   //Local nPosQTNFRE := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QTNFREC" } )
   //Local nPosQTESTO := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QTESTOQ" } )
   //Local nPosQTPERD := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QTPERDA" } )
   //Local nPosQTREAP := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QTREAPR" } )

   Local nPosQTO := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QTDORIG" } )
   Local nPosPED := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_PEDIDO"  } )
   Local nPosNNF := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_NUMNF"   } )
   Local nPosSNF := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_SERNF"   } )
   Local nPosVUO := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_VUORI"   } )
   Local nPosVUT := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_VTORI"   } )
   Local nPosLOT := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_LOTE"    } )
   Local nPosVLD := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_DTVALID" } )
   Local nPosQTP := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_QTDPRO"  } )
   Local nPosRET := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_RETEN"   } )
   Local nPosDEV := aScan(oGetDadSAC:aHeader, {|x| Alltrim(x[2]) == "ZR_ITEMDEV" } )
   Local nY
   Local _cOcorrAnt := Posicione("SZR", 1, xFilial("SZR")+SZS->ZS_NUM  , "ZR_OCORREN")
   Local _cAssunAnt := Posicione("SZR", 1, xFilial("SZR")+SZS->ZS_NUM  , "ZR_ASSUNTO")
   ...
   ElseIf nOpc == 3 //Gravar Atendimento
      ...
      //Preenchimento do tipo do SAC
      If Empty(cTPSac) // aquix
         FWAlertError("Atenção, o 'Tipo de SAC' não foi preenchido.","Sem TIPO SAC")
         Return(Nil)
      Endif

      //Preenchimento do tipo de Desconto
      If Empty(cTpDesc)
         FWAlertError("Atenção, o 'Tipo de Desconto' não foi preenchido.","Sem Tipo de Desconto")
         Return (NIL)
      Endif

      //Preenchimento do Cliente
      If Empty(cSACClie)
         FWAlertError("Atenção, o 'Código do Cliente' não foi preenchido.","Codigo de Cliente Vazio")
         Return (NIL)
      Endif

      //Preenchimento do Contado Cliente
      If Empty(cSACContat)
         FWAlertError("Atenção, o 'Contato no Cliente' não foi preenchido.","Sem Contato do Cliente")
         Return (NIL)
      Endif

      //Preenchimento do Assunto
      If Empty(cSACAssu)
         FWAlertError("Atenção, o 'Código do Assunto' não foi preenchido.","Sem Assunto")
         Return (NIL)
      Endif

      //Preenchimento da Ocorrência
      If Empty(cSACOcor)
         FWAlertError("Atenção, o 'Código da Ocorrência' não foi preenchido.","Ocorrencia nao Preenchida")
         Return (NIL)
      Endif

      //Preenchimento do Problema/Ocorrência
      If Empty(cSACDEsc)
         FWAlertError("Atenção, a 'Ocorrência/Problema' não foi preenchido.","Sem preechimento Ocorrencia")
         Return (NIL)
      Endif

      //Preenchimento dos itens
      If Len(oGetDadSAC:aCols) <= 0 .or. Empty(oGetDadSAC:aCols[1][nPosPRO])
         FWAlertError("Atenção, não existem itens cadastrado para esse SAC.","Sem Itens")
         Return (NIL)
      Else
         lTemLin:=.F.
         lExiIZr := .F. // Controla Itens digitados com quantidade igual a zero
         For nY := 1 To Len(oGetDadSAC:aCols)
            If oGetDadSAC:aCols[nY][Len(oGetDadSAC:aHeader)+1]
               Loop
            End
            lTemLin:=.T.
            If oGetDadSAC:aCols[nY][nPosQTD] <= 0 .and. .NOT. Alltrim(cTpDesc) $ 'NCC e AB-'
               lExiIZr := .T.
               Exit
            Endif
         Next
      ...
      If lExiIZr
         FWAlertError("Atenção, existem itens cadastrado com quantidade igual a zero.","Quantidade Zero")
         Return (NIL)
      Endif
      If .NOT. lTemLin
         FWAlertError("Atenção, não existem linhas validas de itens para esse SAC.","Sem Itens")
         Return (NIL)
      Endif

      //Encaminha SAC - xaqui
      aRetEnc := NfEncamSac(nOpc)
      cDepEnc := aRetEnc[1]
      nRetEnc := aRetEnc[2]
      ...
      Begin Transaction
         ...
         For nY := 1 To Len(oGetDadSAC:aCols)
            ...
            End
                  ...
                  SZR->ZR_FILIAL  := xFilial("SZR")
                  SZR->ZR_NUM     := cSacNum 
                  SZR->ZR_ASSUNTO := cSACAssu
                  SZR->ZR_OCORREN := cSACOcor
                  SZR->ZR_SEQITEM := oGetDadSAC:aCols[nY][nPosSEQ]
                  SZR->ZR_PRODUTO := oGetDadSAC:aCols[nY][nPosPRO]
                  SZR->ZR_QTD     := oGetDadSAC:aCols[nY][nPosQTD]
                  SZR->ZR_QTDORIG := oGetDadSAC:aCols[nY][nPosQTO]
                  SZR->ZR_PEDIDO  := oGetDadSAC:aCols[nY][nPosPED]
                  SZR->ZR_NUMNF   := oGetDadSAC:aCols[nY][nPosNNF]
                  SZR->ZR_SERNF   := oGetDadSAC:aCols[nY][nPosSNF]
                  SZR->ZR_VUORI   := oGetDadSAC:aCols[nY][nPosVUO]
                  SZR->ZR_VTORI   := oGetDadSAC:aCols[nY][nPosVUT]
                  SZR->ZR_LOTE    := oGetDadSAC:aCols[nY][nPosLOT]
                  SZR->ZR_DTVALID := oGetDadSAC:aCols[nY][nPosVLD]
                  SZR->ZR_QTDPRO  := oGetDadSAC:aCols[nY][nPosQTP]
                  SZR->ZR_RETEN   := oGetDadSAC:aCols[nY][nPosRET]
                  SZR->ZR_ITEMDEV := oGetDadSAC:aCols[nY][nPosDEV]
                  SZR->ZR_QAPRRET := oGetDadSAC:aCols[nY][nPosQAPRRT]
                  SZR->ZR_ITEMORI := oGetDadSAC:aCols[nY][nPosITEMOR]
                  SZR->ZR_QTDABAT := oGetDadSAC:aCols[nY][nPosQTDABA]

                  //SZR->ZR_QTNFREC := oGetDadSAC:aCols[nY][nPosQTNFRE]
                  //SZR->ZR_QTESTOQ := oGetDadSAC:aCols[nY][nPosQTESTO]
                  //SZR->ZR_QTPERDA := oGetDadSAC:aCols[nY][nPosQTPERD]
                  //SZR->ZR_QTREAPR :=oGetDadSAC:aCols[nY][nPosQTREAP]
                  MsUnLock()
...
@return variant, return_description
/*/
Static Function NfEncamSac(nOpc)
   Local nOpcEnc   := 0
   Local nSelecao  := 1
   Local aPergs:={}
   Local cOpcao:=""
   ...
   If nOpc == 3
      aEncSAC := {"Diretoria", "Comercial", "Financeiro", "Tecnico", "Expedição", "Viamix - Depósito SP", "Recebimento", "Produção" ,"Suprimentos - Compras", "# Manutencao","- Contabilidade",  "Manter Pendente"}
   Else
      If SZQ->ZQ_FLAG $ 'A/M'
         aEncSAC := {"Diretoria", "Comercial", "Financeiro", "Tecnico", "Expedição", "Viamix - Depósito SP", "Recebimento", "Produção", "Suprimentos - Compras", "# Manutencao","- Contabilidade", "Manter Pendente"}
      Else
         If cDepart $ 'DIR/ATE/INF'
            aEncSAC := {"Atendimento", "Diretoria", "Comercial", "Financeiro", "Tecnico", "Expedição", "Viamix - Depósito SP", "Recebimento", "Produção", "Suprimentos - Compras", "# Manutencao","- Contabilidade", "Manter Pendente"}
         ElseIf cDepart $ 'FIS/REC'
            aEncSAC := {"Atendimento", "Financeiro", "Manter Pendente"}
         Else
            aEncSAC := {"Atendimento", "Manter Pendente"}
         Endif
      Endif
   Endif
   aSort(aEncSAC)
   aAdd(aPergs, {2, "Encaminhamento SAC:", nSelecao, aEncSAC,100, ".T.", .F.})
   If ParamBox(aPergs, "Informe os parâmetros")
      cOpcao:=mv_par01
      nOpcEnc := 1
     If (cOpcao <> 'Man' .AND. cOpcao <> 'Ate')
         //chamMail(mv_par01)
      EndIf
   End
Return({cOpcao, nOpcEnc})
...
//oGrp4Sac   := TGroup():New( 062, 008, /* 152*/ 155, 356, "Dados do Atendimento", oGrp1Sac, CLR_RED, CLR_WHITE, .T., .F. )
//oSay6Sac   := TSay():New( 072, 010, {||"Atendente:"}          , oGrp4Sac, , oFon2Sac, .F., .F., .F., .T., CLR_BLUE, CLR_WHITE, 040, 008)
//oGet7Sac   := TGet():New( 070, 054, {|u| If(PCount() > 0, cGet7Sac := u, cGet7Sac)}, oGrp4Sac, 076,
/*
oSay7Sac   := TSay():New( 072, 146, {||"Hr. Inicio:"}         , oGrp4Sac, , oFon2Sac, .F., .F., .F., .T., CLR_BLUE, CLR_WHITE, 036, 008)
oGet8Sac   := TGet():New( 070, 186, {|u| If(PCount() > 0, cGet8Sac := u, cGet8Sac)}, oGrp4Sac, 036, 010, '', , CLR_BLACK, CLR_WHITE, oFon2Sac, , , .T., "", , , .F., .F., , .T., .F., "", "cGet8Sac", , )
oSayASac   := TSay():New( 088, 010, {||"Assunto:"}            , oGrp4Sac, , oFon2Sac, .F., .F., .F., .T., CLR_BLUE, CLR_WHITE, 040, 008)
