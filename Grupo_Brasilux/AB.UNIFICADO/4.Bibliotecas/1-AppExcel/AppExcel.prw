...
METHOD New(cSheetName) CLASS APPEXCEL
    ...
    ::cDestPath := "C:\TEMP\" //GetTempPath(.T.)
...
@example
	oExcel:Make()
/*/
METHOD Make( _lSched ) CLASS APPEXCEL   
	Local nHandle   := FCreate( ::cFileName , 0 )
	Local nX		:= 0
	Local _lSchedule := Iif( _lSched == Nil, .f., _lSched )
	Local nY
	If nHandle < 0
	...
	FWrite(nHandle,'<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"' + CRLF)
 	...
 	FWrite(nHandle,'          <Author>Administrator</Author>' + CRLF)
    FWrite(nHandle,'          <LastAuthor>Administrator</LastAuthor>' + CRLF)
    FWrite(nHandle,'          <Created>2016-09-28T17:32:36Z</Created>' + CRLF)
   ...
   FWrite(nHandle,'      </Style>' + CRLF)             
    ...
    Else
       _lWait     := .t.
       _lRet      := .f.
       _cComando  := "E:\TOTVS\Protheus11_PRD\Protheus_Data\dirdoc\TI\copiar.bat"
       _cPath     := "E:\TOTVS\Protheus11_PRD\Protheus_Data\dirdoc\TI\"
       
       _lRet := WaitRunSrv(@_cComando, @_lWait, @_cPath )
       
       _cArqOri   := "KPI Vendas " + StrZero( Year( dDataBase ), 4 ) + StrZero( Month( dDataBase ), 2 ) + StrZero( Day( dDataBase ), 2 ) + ".xml"
       _cArqDes   := "KPI Vendas " + StrZero( Year( dDataBase ), 4 ) + StrZero( Month( dDataBase ), 2 ) + StrZero( Day( dDataBase ), 2 ) + ".xlsx"
       _cStrCmd   := ""
       _cStrCmd   += "REM Definicao das pastas de entrada e saida" + CHR(13) + CHR(10)
       _cStrCmd   += "set InputFolder = E:\TOTVS\Protheus11_PRD\Protheus_Data\dirdoc\Comercial\" + CHR(13) + CHR(10)
       _cStrCmd   += "set OutputFolder=E:\TOTVS\Protheus11_PRD\Protheus_Data\dirdoc\Comercial\" + CHR(13) + CHR(10)
       _cStrCmd   += " " + CHR(13) + CHR(10)
       _cStrCmd   += "convertxls /S%InputFolder%" + _cArqOri + " /T%OutputFolder%" + _cArqDes + " /F-4143 /C51 /M1 " + chr(13) + chr(10)
       _cStrCmd   += " " + CHR(13) + CHR(10)
       //MemoWrite( _cPath + 'ConvXLS.BAT', _cStrCmd )
       
       nHandle   := FCreate( '\dirdoc\ti\convXLS.bat', 0 )
       If nHandle < 0
          //LGS#20200207 - Adequação de release 12.1.25 e posteriores
          //Conout("Erro ao criar o arquivo bat.")
          FwLogMSG( "ERROR", , 'CLASSEXCEL', funname(), '', '01', "Erro ao criar o arquivo bat.", 0 )
       EndIf
       FWrite( nHandle, "REM Definicao das pastas de entrada e saida" + CHR(13) + CHR(10) )
       FWrite( nHandle, "set InputFolder = E:\TOTVS\Protheus11_PRD\Protheus_Data\dirdoc\Comercial\" + CHR(13) + CHR(10) )
       FWrite( nHandle, "set OutputFolder=E:\TOTVS\Protheus11_PRD\Protheus_Data\dirdoc\Comercial\" + CHR(13) + CHR(10) )
       FWrite( nHandle, " " + CHR(13) + CHR(10) )
       FWrite( nHandle, "convertxls /S%InputFolder%" + _cArqOri + " /T%OutputFolder%" + _cArqDes + " /F-4143 /C51 /M1 " + chr(13) + chr(10) )
       FWrite( nHandle, " " + CHR(13) + CHR(10) )
