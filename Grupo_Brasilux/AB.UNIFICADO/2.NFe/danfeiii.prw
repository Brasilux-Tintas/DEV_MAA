...
#DEFINE MAXITEMC  072                                                // Máxima de caracteres por linha de produtos/serviços
...
User Function DANFE_P1(cIdEnt,cVal1,cVal2,oDanfe,oSetup,lMostraPerg,_aParam)
Local aArea     := GetArea() 
Local lExistNfe := .F.
DEFAULT lMostraPerg := .T.
...
If ( oDanfe:lInJob ) 
	lEnd := .f.
	DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe)
else
	RptStatus({|lEnd| DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe,@_aParam)},"Imprimindo Danfe...")
endif 
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,cVal1,cVal2,lExistNfe,_aParam)
...
//Local lPerguntou,cAliasD2

lPerguntou := iif(lPerguntar,Pergunte("NFSIGW",.t.),.t.)
If lPerguntou
...
If Len(aGrvSF3)>0 .And. SF3->(FieldPos("F3_CHVNFE"))>0
	...
	Next nI
EndIf
_aParam := {MV_PAR01,MV_PAR02,MV_PAR03,MV_PAR04,MV_PAR05}
RestArea(aArea)
Return(.T.)
...
Local lNFori2       := .T.
//Local lCompleECF    := .F.
Local lEntIpiDev   	:= GetNewPar("MV_EIPIDEV",.T.) /*Apenas para nota de entrada de Devolução de ipi. .T.-Séra destacado no cabeçalho + inf.compl/.F.-Será destacado apenas em inf.compl*/
Local _lQVol
Local _i
...
IIF(Type("oIdent:_DHSaiEnt")=="U","",oIdent:_DHSaiEnt:Text),; //IIF(Type("oIdent:_DSaiEnt")=="U","",oIdent:_DSaiEnt:Text),;//                              oIdent:_DSaiEnt:Text,;
...
If Type("oTransp:_Vol")<>"U"
	If ValType(oTransp:_Vol) == "A"
		...
		_lQVol := Type("oTransp:_Vol[nXAux]:_QVOL:TEXT")=="U"
		For nX := 1 to nLenVol
			nXAux := nX
			...
			nVolume += IIF( !_lQVol, Val( oTransp:_Vol[nXAux]:_QVOL:TEXT ), 0 )
		Next nX
...
For nZ := 1 To nLenDet
	...
	EndIf
	//BRASILUX - Subst qtrib por qtd com
...
//	nPerDesc	:= ((nDesc*100)/nVTotal)
	...
	nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 66)
...
/*	AADD(aAux, AllTrim(TransForm(nValICMST,TM(nValICMST,TamSX3("D2_VALICM")[1],TamSX3("D2_VALICM")[2]))))*/
	...
	//Eliminar zeros à direita	
	for _i := 1 to len(aAux)
		_cDig := "0"    
		_nPos := len(aAux[_i])  
		_lAchou := .f.
		do while (_cDig == "0") .and. !_lAchou
			_cDig := substr(aAux[_i],_nPos,1) 
			if _cDig $ ",$."	
				_lAchou := .t. // Somente elimina zeros à direita se houver casas decimais
			endif 
			_nPos--
		enddo 
		//Cleber (21/01/15), existem produtos com qtde em casas decimais
		  //Para qtde eliminar todos os zeros à direita, os outros campos deixar com 2 casas decimais
		nAux := iif(_i == 1,1,2)
		
			if _nPos < (AT(",",aAux[_i])+nAux)
				_nPos := (AT(",",aAux[_i])+nAux)
			endif 
		aAux[_i] := alltrim(substr(aAux[_i],1,_nPos))
	next 
	//LGS#20200213 - Adequação de release 12.1.25 e posteriores
	...
	aadd(aItens,{;
		...
		oDet[nX]:_Prod:_uCom:TEXT,; //Cleber BRASILUX (21/07/17), subst. de oDet[nX]:_Prod:_utrib:TEXT,;
    ...
    lPontilhado := .T. //.F. Cleber(22/08/14)->sempre colocar pontilhados
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]);
	       .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
		nMaxCod := MaxCod(cAuxItem, 66)
...
//BRASILUX -> RESUMO DE IMPOSTOS PARA RO
//IF (oDestino:_EnderDest:_UF:Text = "RO")
IF MV_PAR04==2 .AND. (oDestino:_EnderDest:_UF:Text = "RO")

	cAliasD2 := GetNextAlias()
	BeginSql Alias cAliasD2
	
	SELECT D2_CLASFIS,D2_PICM,SUM(D2_TOTAL) AS VLRTOTAL,SUM(D2_BASEICM) AS BASEICM,SUM(D2_VALICM) AS VALICM,SUM(D2_VALIPI) AS VALIPI 
	FROM %Table:SD2% SD2  
	WHERE (SD2.%NotDel%) AND (D2_FILIAL = %xFilial:SD2%) AND (D2_DOC = %Exp:aNota[5]%) AND (D2_SERIE = %Exp:aNota[4]%) 
	GROUP BY D2_CLASFIS,D2_PICM ORDER BY D2_CLASFIS 	
	EndSql
	...
	while !eof()
		aadd(aItens,{;
			SubStr(replicate("*",15),1,nMaxCod),;
			SubStr(replicate("*",50),1,nMaxDes),;
			"********",;
			"",;
			"",;
			"",;
		...
		aadd(aItens,{;
			SubStr(replicate("*",15),1,nMaxCod),;
			SubStr("SUBTOTAIS (§ 12, artº 19, Convênio s/ nº de 1970)",1,nMaxDes),;
			"********",;
			"",;
			"",;
			"",;
		...
		nLenDet++
		while !eof()
			cAuxCst := (cAliasD2)->D2_CLASFIS
			aadd(aItens,{;
				SubStr(replicate("*",15),1,nMaxCod),;
				SubStr(".CST do ICMS: "+cAuxCst,1,nMaxDes),;
				"********",;
				"",;
				"",;
				"",;
			...
			nLenDet++
			while !eof() .and. ((cAliasD2)->D2_CLASFIS == cAuxCst)
			
				aadd(aItens,{;
					SubStr(replicate("*",15),1,nMaxCod),;
					SubStr("..Alíquota do ICMS: "+alltrim(TRANSFORM((cAliasD2)->D2_PICM,"999,99"))+"%",1,nMaxDes),;
					"********",;
					"",;
					"",;
					"",;
				...
				aadd(aItens,{;
					SubStr(replicate("*",15),1,nMaxCod),;
					SubStr("..Valor Total dos Produtos: "+ALLTRIM(transform((cAliasD2)->VLRTOTAL,"@E 999,999.99")),1,nMaxDes),;
					"********",;
					"",;
					"",;
					"",;
				...
				aadd(aItens,{;
					SubStr(replicate("*",15),1,nMaxCod),;
					SubStr("..Base de Cálculo do ICMS : "+alltrim(transform((cAliasD2)->BASEICM,"@E 999,999.99")),1,nMaxDes),;
					"********",;
					"",;
					"",;
					"",;
				...
				aadd(aItens,{;
					SubStr(replicate("*",15),1,nMaxCod),;
					SubStr("..ICMS = "+alltrim(transform((cAliasD2)->VALICM,"@E 999,999.99")),1,nMaxDes),;
					"********",;
					"",;
					"",;
					"",;
				...
				aadd(aItens,{;
					SubStr(replicate("*",15),1,nMaxCod),;
					SubStr("..IPI = "+alltrim(transform((cAliasD2)->VALIPI,"@E 999,999.99")),1,nMaxDes),;
					"********",;
					"",;
					"",;
					"",;
...
if !empty(cAux)
	While !Empty(cAux)
		aadd(aMensagem,alltrim(substr(cAux,1,MAXMENLIN)))  
		if len(cAux) > MAXMENLIN
			cAux := substr(cAux,MAXMENLIN+1,len(cAux)-MAXMENLIN)
		else
			cAux := ""
		endif    
	enddo 
...
oDanfe:Say(nLine+265,nBaseTxt+1,"DESTINATARIO /",oFont08N:oFont, , CLR_BLACK, 270 )
oDanfe:Say(nLine+260,nBaseTxt+11,"REMETENTE"     ,oFont08N:oFont, ,CLR_BLACK , 270 )
...
oDanfe:Say(nLine+305,nBaseTxt+7,"FATURA",oFont08N:oFont, ,CLR_BLACK , 270 )
nBaseTxt += 30 
...
oDanfe:Say(nLine+372,nBaseTxt,"CALCULO",oFont08N:oFont, ,CLR_BLACK , 270 )
oDanfe:Say(nLine+360,nBaseTxt+7,"DO",oFont08N:oFont, , CLR_BLACK, 270 )
oDanfe:Say(nLine+370,nBaseTxt+14,"IMPOSTO",oFont08N:oFont, , CLR_BLACK, 270 )
nBaseTxt += 30 
...
oDanfe:Say(nLine+446,nBaseTxt   ,"TRANSPORTADOR/" ,oFont08N:oFont, , CLR_BLACK, 270 )
oDanfe:Say(nLine+438,nBaseTxt+7 ,"VOLUMES"        ,oFont08N:oFont, , CLR_BLACK, 270 )
oDanfe:Say(nLine+448,nBaseTxt+14,"TRANSPORTADOS"  ,oFont08N:oFont, , CLR_BLACK, 270 )
nBaseTxt += 30 
...
oDanfe:Say(nLine+596,nBaseTxt+7,"ISSQN",oFont08N:oFont, , CLR_BLACK, 270 )
nBaseTxt += 30 
...
For nY := 1 To nLenItens
	...
	Else
		...
		for _i := 7 to 14
			nAuxH2 := aColProd[_i][1] + ((aColProd[_i][2] - aColProd[_i][1] - 2) - RetTamTex(aAux[1][_i][nY], oFont07:oFont, oDanfe)) + 2
			oDanfe:Say(nLinha, nAuxH2, aAux[1][_i][nY], oFont07:oFont)
		next 
	EndIf
	
	nLinha := nLinha + 10 
Next nY 
If nL <= MAXITEM .And. Len(aMensagem) > MAXMSG .And. (nFolha == 2) .And. (nLenItens <= MAXITEM) //MAXMSG
	oDanfe:EndPage()
	oDanfe:StartPage()
	nLinha    	:=	181                
	DanfeCab(oDanfe,nPosV,oNFe,oIdent,oEmitente,nFolha,nFolhas,cCodAutSef,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,@nLine,@nBaseCol,@nBaseTxt,aUf)
...
//BRASILUX
If (cNumEmp = "01010101") .and. (oIdent:_TpNf:Text=="1")
    ...
    */
	oDanfe:Box(10,75,150,370)
	oDanfe:Say(25,75, padc("PREZADO CLIENTE, CASO NÃO RECEBA",35),oFont18N:oFont)
	oDanfe:Say(40,75, padc(" OS BOLETOS ATÉ 03 DIAS ANTES DO",35),oFont18N:oFont)
	oDanfe:Say(55,75, padc(" VENCIMENTO FAVOR ENTRAR EM",35),oFont18N:oFont)
	oDanfe:Say(70,75, padc("       CONTATO ATRAVÉS",35),oFont18N:oFont)
	oDanfe:Say(85,75,  "* Site: www.brasilux.com.br",oFont18N:oFont)
	oDanfe:Say(100,75, "* e-mail: brasilux@brasilux.com.br",oFont18N:oFont)
	oDanfe:Say(115,75, "* Tel.: (16) 3383-7000 ou ",oFont18N:oFont)
	oDanfe:Say(130,75, "* FAX: 0800 55 6002",oFont18N:oFont)

	oDanfe:Box(10,400,150,690)
	oDanfe:Say(25,410, padc("ATENÇÃO! As devoluções de",35),oFont18N:oFont)
	oDanfe:Say(40,410, padc("mercadorias deverão ser autorizadas",35),oFont18N:oFont)
	oDanfe:Say(55,410, padc("previamente pelo Departamento",35),oFont18N:oFont)
	oDanfe:Say(70,410, padc("Comercial através do SAC 0800-11-6003",36),oFont18N:oFont)
	oDanfe:Say(85,410,  padl("---------------------",40),oFont18N:oFont)
/*	oDanfe:Say(100,410,  padl("A Brasilux NÃO ENVIA boleto de cobrança",40),oFont18N:oFont)
	oDanfe:Say(115,410, padl("por e-mail, exceto por solicitação do  ",40),oFont18N:oFont)
	oDanfe:Say(130,410, padl("cliente.                               ",40),oFont18N:oFont)*/
		...
		aAux := {}
			...
			dbselectarea("ZZO")
			dbsetorder(1)
			dbseek(xFilial("ZZO")+SF2->F2_PEDIDO)
			if found()       
				aadd(aAux,{"EMBALAGEM","QTDE","VOLUMES"})
    		endif 
			while !eof() .and. (xFilial("ZZO") == ZZO->ZZO_FILIAL) .AND. (ZZO->ZZO_PEDIDO == SF2->F2_PEDIDO)
...
/*				aadd(Atail(aAux)[1],SubStr(substr(ZZO->ZZO_EMBA,1,2)+"-"+Alltrim(SubStr(cAuxEmba,1,18)), 1, 16))
				aadd(Atail(aAux)[2],Transform(ZZO->ZZO_QTDE,"999,999"))
				aadd(Atail(aAux)[3],Transform(ZZO->ZZO_QTDVOL, "999,999"))    */

           		dbselectarea("ZZO")
           		dbskip()
          	enddo 
          	if len(aAux) > 0
          		_nAuxV := 185+15*(len(aAux)+1)
          		oDanfe:Say(180,80,	"PEDIDO "+SF2->F2_PEDIDO+" - VOLUMES",oFont18N:oFont)
				oDanfe:Box(185,75,_nAuxV,450)

				for _i := 1 to len(aAux)    
					
					oDanfe:Say(185+15*_i,80, aAux[_i,1],oFont18N:oFont)
					oDanfe:Say(185+15*_i,275, aAux[_i,2],oFont18N:oFont)
					oDanfe:Say(185+15*_i,iif(_i == 1,375,395), aAux[_i,3],oFont18N:oFont)
				next 

//				"PEDIDO "+SF2->F2_PEDIDO+" - VOLUMES",{"L","R","R"},2)
          	endif 
	...
	If (MV_PAR05 == 1) .AND. (mod(nFolhas,2)=0)
		oDanfe:StartPage()
		oDanfe:EndPage()
	EndIf
endif 
...
//		oDanfe:SayBitmap(005,075,cLogoD,095,096)
		oDanfe:SayBitmap(005,075,cLogoD,100,28)
	Else
//		oDanfe:SayBitmap(005,075,cLogo,095,096)
		oDanfe:SayBitmap(005,075,cLogo,100,28)
	EndIf
Endif
...
If nFolha == 1
	...
	oDanfe:Say(nLine+568,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, ,CLR_BLACK , 270 )
	nBaseTxt += 30 
	aColProd := {}
	nAux     := nBaseCol + 30
...
/*	AADD(aColProd, {nAux, nAux + aTamCol[11]}) // "BC.ICMS ST"
	nAux += aTamCol[11]*/
	AADD(aColProd, {nAux, nAux + aTamCol[11]}) // "VLR ICMS"
	nAux += aTamCol[11]
//	AADD(aColProd, {nAux, nAux + aTamCol[16]}) // "VLR ICMS ST"
 //	nAux += aTamCol[16]
	AADD(aColProd, {nAux, nAux + aTamCol[12]}) // "VALOR IPI"
	nAux += aTamCol[12]
	AADD(aColProd, {nAux, nAux + aTamCol[13]}) // "ICMS"
	nAux += aTamCol[13]
	AADD(aColProd, {nAux, nAux + aTamCol[14]}) // "IPI"
...
/*	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[16])
	...
	oDanfe:Box(nLine+454, nAux2, nLine+461, nAux2 + aTamCol[13] + aTamCol[14])
...
Else
	...
	//	oDanfe:Box(nLine+454,nBaseCol,MAXBOXV,nBaseCol+30)
		...
		oDanfe:Say(360,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, , CLR_BLACK, 270 )
		nBaseTxt += 30 
		aColProd := {}
		nAux     := nBaseCol + 30
		AADD(aColProd, {nAux, nAux + aTamCol[1]}) //"COD. PROD"
...
/*		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[16])
	...
	//	oDanfe:Box(nLine+454,nBaseCol,MAXBOXV,nBaseCol+30)
		...
		oDanfe:Say(nLine+768,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, , CLR_BLACK, 270 )
		nBaseTxt += 30 
		aColProd := {}
		nAux     := nBaseCol + 30
		AADD(aColProd, {nAux, nAux + aTamCol[1]}) //"COD. PROD"
...
If nFolha ==1
	...
	oDanfe:Say(MAXBOXV-25,nBaseTxt+1,"DADOS",oFont08N:oFont, , CLR_BLACK, 270 )
	oDanfe:Say(MAXBOXV-13,nBaseTxt+11,"ADICIONAIS"    ,oFont08N:oFont, ,CLR_BLACK , 270 )
	nBaseTxt += 30 
	...
	nQtdeLinhas := Min(nLenMensagens, MAXMSG)
	For nX := 1 To nQtdeLinhas
		oDanfe:Say(nLin,nBaseTxt,aMensagem[nX],oFont07:oFont)
		nLin:= nLin+10
	Next nX
	
	If nLenMensagens > nQtdeLinhas
		nPosMsg := nQtdeLinhas+1
	EndIf 
...
*/
	If nAux < MAXBOXH
		aTamCol[2] += MAXBOXH - nAux
	EndIf
   	If nAux > MAXBOXH               
		aTamCol[2] -= nAux - MAXBOXH
	EndIf
