...
±±?Desc     =Esta rotina monta um html com os saldos negativos            ±±
±±?         =          											          ?±±
±±?         =   						 								  ?±±
...
Local dData   	:=DATE()                    
Local cAssunto	:= "WorkFlow dos produtos com saldo negativo. Empresa "+_codEmp+ " Filial "+AllTrim(_codFil)+". Data: "+DTOC(dData)
   
Local cTitulo 	:= "Saldos negativos"
...
//	Filial, numero sc, item sc, dt necessidade, emissao, codigo produto, produto, qtd, solicitante
cQuery := "	SELECT B1_COD, B1_DESC, B1_TIPO, B2_LOCAL, B2_QATU  " +cQLinha
...
cQuery += "	AND B2_QATU < -1 AND B1_TIPO <> 'MO'				" +cQLinha
...
While !EOF()
	aadd(_aItens,{TMPWF->B1_COD,     TMPWF->B1_DESC, TMPWF->B1_TIPO,         TMPWF->B2_LOCAL,      TMPWF->B2_QATU})
	//					 1						 2				   3				     4				      5				          6				      7				          8		             9                           
	dbSkip() 
EndDo

   
_aTitulos := 	{"Código", 	"Descrição", "Tipo", 	"Armazém", 	"Quantidade Atual"}
