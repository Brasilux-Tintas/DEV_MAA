...
Static Function Consult(oDlg,cPedido,odbTree)
	Local cWhere:= ""
	Private TCQ  := GetNextAlias()
	Private cBip := Array(500)
	Private cLastDate[1]
	...
	cWhere += " SC6.C6_NUM    ='" + cPedido  + "' AND "
	cWhere += " SC6.D_E_L_E_T_ ='' "
	If SUBSTR(cTransp,0,5) == "00700" .OR. SUBSTR(cTransp,0,5) == "00573"	
		cWhere += " AND SUBSTRING(C6_PRODUTO,4,2) NOT IN('55') "
	EndIf
	cWhere += "%"
	...
	If cPedido <> ' ' 
		...
		SELECT SC6.C6_ITEM, SC6.C6_PRODUTO, LEN(SC6.C6_PRODUTO) AS cProduto, SC6.C6_DESCRI, LEN(SC6.C6_DESCRI) AS cDescri, SC6.C6_QTDVEN, ISNULL(ZZK.ZZK_CODIGO, '-') AS ZZK_CODIGO, ISNULL(SUM(ZZK.ZZK_QUANT),0) AS ZZK_QUANT, MAX(ZZJ.ZZJ_DTFIM) AS DATAFIM, 	SZG.ZG_LOCALIZ, ZZJ.ZZJ_USER AS USUARIO, ISNULL(SZH.ZH_NOME,'') AS CUSUARIO, CASE WHEN ZZJ.ZZJ_OBSPED <>'' THEN 'S' ELSE 'N' END AS ZZJ_TRANSF 
		FROM %Table:SC6% SC6 
		LEFT JOIN %Table:ZZK% ZZK ON ZZK.ZZK_FILIAL = SC6.C6_FILIAL AND ZZK.ZZK_PEDIDO = SC6.C6_NUM AND ZZK.ZZK_PRODUT = SC6.C6_PRODUTO AND ZZK.%notdel% 
		LEFT JOIN %Table:ZZJ% ZZJ ON ZZJ.ZZJ_FILIAL = ZZK.ZZK_FILIAL AND ZZJ.ZZJ_CODIGO = ZZK.ZZK_CODIGO AND ZZJ.%notdel% 
		LEFT JOIN %Table:SZG% SZG ON SZG.ZG_FILIAL = SC6.C6_FILIAL AND SUBSTRING(SZG.ZG_PEDIDO,3,6) = SC6.C6_NUM AND SZG.%notdel% 
		LEFT JOIN %Table:SZH% SZH ON SZH.ZH_FILIAL = SZH.ZH_FILIAL AND SZH.ZH_CODIGO = ZZJ.ZZJ_USER AND SZH.%notdel% 
		WHERE %exp:cWhere%
		GROUP BY SC6.C6_ITEM, SC6.C6_PRODUTO, SC6.C6_DESCRI, SC6.C6_QTDVEN, ZZK.ZZK_CODIGO, ZZJ.ZZJ_DTFIM, SZG.ZG_LOCALIZ, ZZJ.ZZJ_OBSPED, ZZJ.ZZJ_USER, SZH.ZH_NOME
		ORDER BY SC6.C6_ITEM, ZZK.ZZK_CODIGO
