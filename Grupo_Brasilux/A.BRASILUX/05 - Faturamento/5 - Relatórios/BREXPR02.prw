...
PRIVATE CbTxt
PRIVATE Titulo := "Relatorio rotatividade do pedido"
PRIVATE cDesc1:=""
PRIVATE cDesc2:=""
PRIVATE cDesc3 := "" 
...
PRIVATE nomeprog:= "BREXPR02"
PRIVATE aLinha  := { },nLastKey := 0
PRIVATE cPerg   := "BREXPR02"
Private Total := 0  
Private xFlag := 0
//UAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA¢¯
...
wnrel:= "BREXPR02"            //Nome Default do relatorio em Disco
...
cQuery :="WITH TMPLOG AS (SELECT R_E_C_N_O_ AS REG,ZZB_PEDIDO,ZZB_DATLOG,ZZB_HORLOG "
cQuery +="FROM "+RetSqlName("ZZB")+" ZZB WITH (NOLOCK) "
cQuery +="WHERE (ZZB_FILIAL = '"+xFilial("ZZB")+"') AND (ZZB_IDLOG IN ('AOU', 'APR')) AND (ZZB_DATLOG BETWEEN ('"+dTos(MV_PAR01)+"') AND ('"+dTos(MV_PAR02)+"'))), "
...
cQuery1 :="WITH TMPLOG AS (SELECT R_E_C_N_O_ AS REG,ZZB_PEDIDO,ZZB_DATLOG,ZZB_HORLOG "
cQuery1 +="FROM "+RetSqlName("ZZB")+" ZZB WITH (NOLOCK) "
cQuery1 +="WHERE (ZZB_FILIAL = '"+xFilial("ZZB")+"') AND (ZZB_IDLOG IN ('AOU', 'APR')) AND (ZZB_DATLOG BETWEEN ('"+dTos(MV_PAR01)+"') AND ('"+dTos(MV_PAR02)+"'))), "
...
cQuery1 +="WHERE (SF2.R_E_C_N_O_ IS NOT NULL) AND (F2_DUPL > '') AND (C5_TIPO = 'N') AND (SUBSTRING(D2_COD,4,2)) NOT IN ('97','98','99') AND (C5_PEDPROG = '') AND (F2_TRANSP <> '00700' AND F2_TRANSP <> '00573' )) "
