...
// Jose 11/11/2024   Acesso empresas 010106 ou 010108
     If !(alltrim(cNumEmp) == "01010106") .and. !(alltrim(cNumEmp) == "01010108")
          cMens1 := "Acesso deverá ser feito pelo Deposito Araraquara ou Guarulhos!"
...
// Jose 11/11/2024 até aqui
     ...
     oDlg1Env   := MSDialog():New( 140, 264, 680, 980, "Gera Pedido de Devolução para a Matriz", , , .F., , , , , , .T., , , .T. )
     ...
     oGet6Env   := TComboBox():New( 010, 242, {|u| If(PCount() > 0, cGet6Env := u, cGet6Env)}, {"", "00354"}, 050, 016, oGrp1Env, , , , CLR_BLACK, CLR_WHITE, .T., oFontEnv, "", , , , ,  , , cGet6Env ) // {"", "023818", "052817"}
	...
	Begin Transaction    
		...
		For nX := 1 To Ceiling(Len(aTrPallet)/150)       
		  ...
		  //aAdd(aCabec,{"C5_NUM"        ,		_cNumPed												,.f.})
       		aAdd(aCabec,{"C5_TIPO"       ,		"B"              										,Nil}) // Jose 22/11/2024 "D"  
	    ...
	    cQry1 += " WHERE ZZJ.ZZJ_FILIAL = '010101' " // '"+xFilial("ZZJ")+"'"  jose 11/10/2024
...
//  jOSE 11/11/2024 TIRADO SET ZZJ_FLAG ='5' E SUBSTITUIIDO POR SET ZZJ_LOCORI = ZZJ_LOCALI
   		cQry1 := " "
   		cQry1 += " UPDATE "+RetSqlName("ZZJ")+" SET ZZJ_LOCORI = CASE WHEN  '"+xFilial("ZZJ")+"' = '010108' THEN 'A3' ELSE 'G3' END, ZZJ_OBSPED ='"+cNPedidos+"', ZZJ_CARGA ='"+cCodCARGA+"' FROM "+RetSqlName("ZZJ")+"  WHERE ZZJ_CODIGO IN "+cPallets "
