...
User Function PEPARADO()
    ...
    Local cPara    	:= "kellydiniz@brasilux.com.br,vendas@brasilux.com.br,pcp@brasilux.com.br,expedicao@brasilux.com.br,tecnicowfpedparado@brasilux.com.br"
    //Local cPara    	:= "andre@brasilux.com.br"
    Local cAssunto  := "Pedidos aprovados e não faturados há mais de 7 dias"
    Local cMsg		:= ""
    Local cQuery    := ""
    Local cQuery1   := ""
    //Local TM1, TM2                                                                          
    ...
    cQuery := ""
    cQuery += " SELECT DISTINCT C5_NUM,C5_CLIENT,A1_NOME,SUBSTRING(C5_DTAPR,7,2)+'/'+SUBSTRING(C5_DTAPR,5,2)+'/'+SUBSTRING(C5_DTAPR,1,4) AS EMISSAO ,DATEDIFF(D,C5_DTAPR,GETDATE()) AS 'DIAS', "
    ...
    cQuery += " AND DATEDIFF(D,CASE WHEN C5_PEDPROG = '' THEN C5_DTAPR ELSE C5_PEDPROG END,GETDATE()) > 7 "
    cQuery += " ORDER BY DATEDIFF(D,C5_DTAPR,GETDATE()) DESC "
