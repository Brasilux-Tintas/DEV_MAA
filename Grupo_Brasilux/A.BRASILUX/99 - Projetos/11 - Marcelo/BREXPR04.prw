...
User Function BREXPR04()
//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Define Variaveis                                             Ё
//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
PRIVATE CbTxt
PRIVATE Titulo := "Relatorio de tempo de rotatividade PС"
PRIVATE cDesc1:=""
PRIVATE cDesc2:=""
PRIVATE cDesc3 := "" 
...
PRIVATE nomeprog:= "BREXPR04"
PRIVATE aLinha  := { },nLastKey := 0
PRIVATE cPerg   := "BREXPR04"
Private Total := 0  
Private xFlag := 0
//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
...
wnrel:= "BREXPR04"            //Nome Default do relatorio em Disco
...
cQuery +="LEFT OUTER JOIN "+RetSqlName("SC6")+" SC6 WITH (NOLOCK) ON (SC6.D_E_L_E_T_ <> '*') AND (C6_FILIAL = '"+xFilial("SC6")+"') AND (LEN(C6_PRODUTO) = 12) AND (SUBSTRING(C6_PRODUTO,4,2) IN ('97','98','99') AND (C6_NUM = ZZB_PEDIDO)) "
...
cQuery +="TMP AS (SELECT DISTINCT ZZB_PEDIDO AS PEDIDO,F2_EMISSAO,F2_HORA,REGZB = ISNULL((SELECT TOP 1 REG FROM TMPLOG WHERE (ZZB_PEDIDO = SF2.F2_PEDIDO) ORDER BY ZZB_DATLOG,ZZB_HORLOG),0) "
cQuery +="FROM TMPLOG "
cQuery +="LEFT OUTER JOIN "+RetSqlName("SD2")+" SD2 WITH (NOLOCK) ON (SD2.D_E_L_E_T_ <> '*') AND (D2_FILIAL = '"+xFilial("SD2")+"') AND (D2_PEDIDO = ZZB_PEDIDO) "
...
cQuery +="WHERE (SF2.R_E_C_N_O_ IS NOT NULL) AND (F2_DUPL > '') AND (C5_TIPO = 'N') AND (C5_PEDPROG = '') AND (SUBSTRING(D2_COD,4,2)) NOT IN ('97','98','99') AND (F2_TRANSP = '00700' OR F2_TRANSP = '00573')) "
...
cQuery1 +="LEFT OUTER JOIN "+RetSqlName("SC6")+" SC6 WITH (NOLOCK) ON (SC6.D_E_L_E_T_ <> '*') AND (C6_FILIAL = '"+xFilial("SC6")+"') AND (LEN(C6_PRODUTO) = 12) AND (SUBSTRING(C6_PRODUTO,4,2) IN ('97','98','99') AND (C6_NUM = ZZB_PEDIDO)) "
...
cQuery1 +="TMP AS (SELECT DISTINCT ZZB_PEDIDO AS PEDIDO,F2_EMISSAO,F2_HORA,REGZB = ISNULL((SELECT TOP 1 REG FROM TMPLOG WHERE (ZZB_PEDIDO = SF2.F2_PEDIDO) ORDER BY ZZB_DATLOG,ZZB_HORLOG),0) "
cQuery1 +="FROM TMPLOG "
cQuery1 +="LEFT OUTER JOIN "+RetSqlName("SD2")+" SD2 WITH (NOLOCK) ON (SD2.D_E_L_E_T_ <> '*') AND (D2_FILIAL = '"+xFilial("SD2")+"') AND (D2_PEDIDO = ZZB_PEDIDO) "
...
cQuery1 +="WHERE (SF2.R_E_C_N_O_ IS NOT NULL) AND (F2_DUPL > '') AND (C5_TIPO = 'N') AND (C5_PEDPROG = '') AND (SUBSTRING(D2_COD,4,2)) NOT IN ('97','98','99') AND (SUBSTRING(D2_COD,4,2)) NOT IN ('97','98','99') AND (F2_TRANSP <> '00700' AND F2_TRANSP <> '00573')) "
...
While !Eof()                 	                                                                         
	...
	cCountR +=1
EndDo                	                                                                         
@ _nLin,023 psay "Periodo de "+Substr(dtos(MV_PAR01),7,2)+"/"+Substr(dtos(MV_PAR01),5,2)+"/"+Substr(dtos(MV_PAR01),1,4)+" atИ "+Substr(dtos(MV_PAR02),7,2)+"/"+Substr(dtos(MV_PAR02),5,2)+"/"+Substr(dtos(MV_PAR02),1,4)
@ _nLin++     
@ _nLin,000 psay "SЦo Paulo"
@ _nLin++     
