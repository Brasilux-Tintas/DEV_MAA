...
User Function BREXPR06()
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Define Variaveis                                             ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
PRIVATE CbTxt
PRIVATE Titulo := "Relatorio programado Pó"
PRIVATE cDesc1:=""
PRIVATE cDesc2:=""
PRIVATE cDesc3 := "" 
...
PRIVATE nomeprog:= "BREXPR06"
PRIVATE aLinha  := { },nLastKey := 0
PRIVATE cPerg   := "BREXPR06"
Private Total := 0  
Private xFlag := 0
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
wnrel:= "BREXPR06"            //Nome Default do relatorio em Disco
...
cQuery :="SELECT DISTINCT C5_NUM AS PEDIDO,DATEDIFF(MINUTE,CONVERT(DATETIME,SUBSTRING(C5_PEDPROG,1,4)+'-'+SUBSTRING(C5_PEDPROG,5,2)+'-'+SUBSTRING(C5_PEDPROG,7,2)+' '+'23:59',102),CONVERT(DATETIME,SUBSTRING(F2_EMISSAO,1,4)+'-'+SUBSTRING(F2_EMISSAO,5,2)+'-'+SUBSTRING(F2_EMISSAO,7,2)+' '+SUBSTRING(F2_HORA,1,2)+':'+SUBSTRING(F2_HORA,4,2),102)) AS TEMPO, "
cQuery +="C5_PEDPROG AS DTINI, F2_EMISSAO AS DTFIM, F2_HORA AS HRFIM, F2_PEDIDO AS PEDIDO "
...
cQuery +="LEFT OUTER JOIN "+RetSqlName("SC6")+" SC6 WITH (NOLOCK) ON (SC6.D_E_L_E_T_ <> '*') AND (C5_FILIAL = C6_FILIAL ) AND (C5_NUM = C6_NUM) " 
...
cQuery +="WHERE F2_FILIAL = '"+xFilial("SF2")+"' AND (C5_PEDPROG BETWEEN  ('"+dTos(MV_PAR01)+"') AND ('"+dTos(MV_PAR02)+"'))  AND C5_PEDPROG > '' AND (F2_DUPL > '') AND (C5_TIPO = 'N') AND ZZB_IDLOG IN ('AOU', 'APR') AND F2_TRANSP = '00700' AND F2_HORA <> '' AND F2_EMISSAO <> '' AND LEN(C6_PRODUTO) = '12' AND SUBSTRING(C6_PRODUTO,4,2) IN ('97','98','99')  "
cQuery +="ORDER BY PEDIDO"

TCQUERY cQuery ALIAS "TCQ" NEW
dbselectarea("TCQ")

cQuery1 :="SELECT DISTINCT C5_NUM AS PEDIDO,DATEDIFF(MINUTE,CONVERT(DATETIME,SUBSTRING(C5_PEDPROG,1,4)+'-'+SUBSTRING(C5_PEDPROG,5,2)+'-'+SUBSTRING(C5_PEDPROG,7,2)+' '+'23:59',102),CONVERT(DATETIME,SUBSTRING(F2_EMISSAO,1,4)+'-'+SUBSTRING(F2_EMISSAO,5,2)+'-'+SUBSTRING(F2_EMISSAO,7,2)+' '+SUBSTRING(F2_HORA,1,2)+':'+SUBSTRING(F2_HORA,4,2),102)) AS TEMPO, "
cQuery1 +="C5_PEDPROG AS DTINI, F2_EMISSAO AS DTFIM, F2_HORA AS HRFIM, F2_PEDIDO AS PEDIDO "
...
cQuery1 +="LEFT OUTER JOIN "+RetSqlName("SC6")+" SC6 WITH (NOLOCK) ON (SC6.D_E_L_E_T_ <> '*') AND (C5_FILIAL = C6_FILIAL ) AND (C5_NUM = C6_NUM) "
...
cQuery1 +="WHERE F2_FILIAL = '"+xFilial("SF2")+"' AND (C5_PEDPROG BETWEEN ('"+dTos(MV_PAR01)+"') AND ('"+dTos(MV_PAR02)+"'))  AND C5_PEDPROG > '' AND ZZB_IDLOG IN ('AOU', 'APR') AND (F2_DUPL > '') AND (C5_TIPO = 'N') AND F2_TRANSP <> '00700' AND F2_HORA <> '' AND F2_EMISSAO <> '' AND LEN(C6_PRODUTO) = '12' AND SUBSTRING(C6_PRODUTO,4,2) IN ('97','98','99') "
