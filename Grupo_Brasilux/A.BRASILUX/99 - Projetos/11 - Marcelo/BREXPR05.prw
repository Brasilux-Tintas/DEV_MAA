...
User Function BREXPR05()
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Define Variaveis                                             ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
PRIVATE CbTxt
PRIVATE Titulo := "Relatorio de tempo  programado"
PRIVATE cDesc1:=""
PRIVATE cDesc2:=""
PRIVATE cDesc3 := "" 
...
PRIVATE nomeprog:= "BREXPR05"
PRIVATE aLinha  := { },nLastKey := 0
PRIVATE cPerg   := "BREXPR05"
Private Total := 0  
Private xFlag := 0
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
wnrel:= "BREXPR05"            //Nome Default do relatorio em Disco
...
cQuery :="SELECT DISTINCT DATEDIFF(MINUTE,CONVERT(DATETIME,SUBSTRING(C5_PEDPROG,1,4)+'-'+SUBSTRING(C5_PEDPROG,5,2)+'-'+SUBSTRING(C5_PEDPROG,7,2)+' '+'23:59',102),CONVERT(DATETIME,SUBSTRING(F2_EMISSAO,1,4)+'-'+SUBSTRING(F2_EMISSAO,5,2)+'-'+SUBSTRING(F2_EMISSAO,7,2)+' '+SUBSTRING(F2_HORA,1,2)+':'+SUBSTRING(F2_HORA,4,2),102)) AS TEMPO, "
...
cQuery +="WHERE F2_FILIAL = '"+xFilial("SF2")+"' AND (C5_PEDPROG BETWEEN ('"+dTos(MV_PAR01)+"') AND ('"+dTos(MV_PAR02)+"')) AND (F2_DUPL > '') AND (C5_TIPO = 'N') AND C5_PEDPROG > '' AND ZZB_IDLOG IN ('AOU', 'APR') AND F2_TRANSP = '00700' AND F2_HORA <> '' AND F2_EMISSAO <> '' "
cQuery +="ORDER BY F2_PEDIDO"

TCQUERY cQuery ALIAS "TCQ" NEW
dbselectarea("TCQ")

cQuery1 :="SELECT DISTINCT DATEDIFF(MINUTE,CONVERT(DATETIME,SUBSTRING(C5_PEDPROG,1,4)+'-'+SUBSTRING(C5_PEDPROG,5,2)+'-'+SUBSTRING(C5_PEDPROG,7,2)+' '+'23:59',102),CONVERT(DATETIME,SUBSTRING(F2_EMISSAO,1,4)+'-'+SUBSTRING(F2_EMISSAO,5,2)+'-'+SUBSTRING(F2_EMISSAO,7,2)+' '+SUBSTRING(F2_HORA,1,2)+':'+SUBSTRING(F2_HORA,4,2),102)) AS TEMPO, "
...
cQuery1 +="WHERE F2_FILIAL = '"+xFilial("SF2")+"' AND (C5_PEDPROG BETWEEN ('"+dTos(MV_PAR01)+"') AND ('"+dTos(MV_PAR02)+"')) AND (F2_DUPL > '') AND (C5_TIPO = 'N')AND C5_PEDPROG > '' AND ZZB_IDLOG IN ('AOU', 'APR') AND F2_TRANSP <> '00700' AND F2_HORA <> '' AND F2_EMISSAO <> '' "
