...
User Function BRFATA02()
    ...
    //If PSWADMIN( , , RetCodUsr() ) != 0
     ...
     If SubStr(cAcessos, 8, 1) $ '*'
		...
		aAdd(aRotina, { OemToAnsi('Estorna Bordero de Pedido'), 'u_BREstBor()', 0, 9} ) //Reprocessa relatório de faltas pra um determinado pedido
     Endif
//------------   JOSE 25/10/2022 --------
...
User Function BRESTBOR()
   ...
   If SZF->ZF_FLAG = 'D'
      Messagebox("Status do Bordero -> "+SZF->ZF_CODIGO+" não permite estorno !!","Atenção...",48)  
      Return
   Endif    

   If !MsgYesNo("Confirma o estorno e demontagem dos itens separados para o bordero "+SZF->ZF_CODIGO+" ?. Todo o movimento de separação será apagado e os pallets serão desmontados.", "Pergunta...")
      Return
   Endif    
   ...
   Begin Transaction
      ...
      Messagebox("Estornado Status do Bordero -> "+SZF->ZF_CODIGO+" !!","Atenção...",48)  
...
//        cQtdea   = round( ( Round( ( cPercen1 / 100 ),2) * cQtde ),2)
          ...
          //Aba 01 
          oFWMsExcel:AddworkSheet("FechaPedido") //Não utilizar número junto com sinal de menos. Ex.: 1-
          //Criando a Tabela
          ...
          oFWMsExcel:AddColumn("FechaPedido",ccabecalho,"VOLUME PED"     , 2)     // SC5.C5_VOLUME1       Volume que está no pedido      //"VOLUME PED"        // aTotPed1[nX][3]
          oFWMsExcel:AddColumn("FechaPedido",ccabecalho,"QT IT CONTR EST", 2, 1)  // SB1.B1_ESTOQUE="S"   Quandidade de Itens que são controlados estoque, ou seja, esta com S neste campo     // aTotPed1[nX][8]
          oFWMsExcel:AddColumn("FechaPedido",ccabecalho,"QT IT PEDIDO"   , 2)     //                      Quantidade de Itens que tem no pedido  // aTotPed1[nX][6]
          oFWMsExcel:AddColumn("FechaPedido",ccabecalho,"QT IT ATEND "   , 2, 1)  //                      Quantidade de Itens que deu para atender com a quantidade que está em estoque  //  INT( aTotPed1[nX][7] )
...
//            INT( aTotPed1[nX][2]  ),;  // Jose 07/02/2024  aTotPed1[nX][10];
          ...
          oExcel := MsExcel():New()           //Abre uma nova conexão com ExcelVOLUME
       ...
       EndIf
       IF cPrim = 0
       	MsgStop("Atenção, Bordero Vazio, verifique antes de prosseguir !!!")
       Endif
      Endif
      RestArea(aAreaa)
...
Static Function fTExcel(cGet2Env)
       ...
       If Empty(Alltrim(cGet2Env))
 			 Messagebox("Informe o código do Borderô de pedidos !","Atenção...",48)
			 _lRet := .f.
       Else
          _lRet := .t.
       Endif
